...
# Models sometimes echo the transcript's role labels (e.g. output starting
# with "[PLANNER]"). Role-constant tokens repeated every turn bias the
# role-stability and KLRD metrics, so strip leading tags before measuring.
_ROLE_TAG = re.compile(r"^\s*\[?\**(?:PLANNER|EXECUTOR|CRITIC)\**\]?\s*[:\-]?\s*", re.I)


def _clean(text):
    return _ROLE_TAG.sub("", text)
...
def build_role_resources(trajs, embedder, tokenizer):
    ...
    for role in ROLES:
        first_turns = []
        for tr in trajs:
            texts = [_clean(t["text"]) for t in tr["turns"] if t["role"] == role]
            if texts:
...
def features_for_traj(traj, refs, cents, embedder, tokenizer):
    ...
    for role in ROLES:
        texts = [_clean(t["text"]) for t in traj["turns"] if t["role"] == role]
        if len(texts) < 2:
            continue
