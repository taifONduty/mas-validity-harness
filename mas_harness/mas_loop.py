...
def run_record(record, backbone, max_shards=None, sleep_s=0.0):
    ...
    # Final-answer elicitation: the model is never otherwise told that the
    # last shard WAS the last (pilot record 968 solved the task but refused
    # to commit, waiting for more info). This extra executor call is used
    # ONLY for outcome scoring; it is NOT appended to `turns`, so all
    # process metrics still see the natural, unprompted conversation.
    ...
    final_answer = backbone.chat(messages)
